...
function romanTime(time) {
    if (!time) throw TypeError('Неверное время');
    var splits = time.split(':');
    var hours = parseInt(splits[0], 10);
    var minutes = parseInt(splits[1], 10);
    if (!isValidPeriod(hours, minutes)) throw TypeError('Неверное время');
...
function getRoman(number) {
    if (number in romanByDigit) return romanByDigit[number];
    if (number in romanByDozens) return romanByDozens[number];
