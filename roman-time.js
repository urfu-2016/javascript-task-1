...
function romanTime(time) {
	...
	if (parts.length != 2)
		throw new TypeError('The parameter is divided by colons into a number of parts other than two');
	
	var hours = parseInt(parts[0], 10);
	var minutes = parseInt(parts[1], 10);
	
	if (!isIntegerFromRange(hours, 0, 23))
		throw new TypeError('The hours part is not a valid natural number or lies outside of [0; 23]');
	if (!isIntegerFromRange(minutes, 0, 59))
		throw new TypeError('The minutes part is not a valid natural number or lies outside of [0; 59]');
