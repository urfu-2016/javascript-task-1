...
function convertToRoman(hours, minutes, halfTime) {
    ...
    if (Number(halfTime) === 0) {
        halfTime[0] = 'N';
        halfTime[1] = 'N';
    } else {
        halfTime[0] = leftNumbers[Number(hours[0])].concat(rightNumbers[Number(hours[1])]);
        halfTime[1] = leftNumbers[Number(minutes[0])].concat(rightNumbers[Number(minutes[1])]);
    }
